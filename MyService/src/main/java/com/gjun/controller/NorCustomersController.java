...
//			}
	...
	public List<Customers> AllCustomers3() {
		String sql = "select CustomerID,CompanyName,Address,Phone,Country from Customers ";
		ResultSetExtractor<List<Customers>> resultSetExtractor = JdbcTemplateMapperFactory.newInstance()
	...
	public ResponseEntity<Object> customersById(@PathVariable("cid") String customerId) {
		...
		try {
			Customers result = jdbcTemplate.queryForObject(
					"Select CustomerID,CompanyName,Address,Phone," + "Country From Customers Where CustomerID=?",
	...
	public ResponseEntity<Object> get2(@PathVariable("cid") String cid) {
		try {
			...
			Customers customers = jdbcTemplate.queryForObject(sql,
					new BeanPropertyRowMapper<Customers>(Customers.class), cid);
	...
	@GetMapping(path = "/nor/customers/id/{cid}/rawdata3" , produces="application/json;charset=UTF-8" )
